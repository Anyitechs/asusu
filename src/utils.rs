...
pub(crate) fn take_while(accept: impl Fn(char) -> bool, s: &str) -> (&str, &str) {
    let extracted_end = s.char_indices().find_map(|(idx, c)| if accept(c) { None } else { Some(idx) }).unwrap_or_else(|| s.len());
...
pub(crate) fn take_while1(accept: impl Fn(char) -> bool, s: &str, error_msg: String,) -> Result<(&str, &str), String> {
...
mod tests {
    ...
    #[test]
    fn do_not_extract_anything_when_input_is_empty() {
